...
- Running `git-spice ls` in any worktree shows the same stack topology
- Creating a branch with `git-spice bc` in one worktree is immediately visible in all others
...
git-spice cc -m "Implement base data model"
```
...
# Make changes, stage them
git-spice cc -m "Add API endpoints for data model"
```
...
When `git-spice cc`, `git-spice ca`, or `git-spice sr` triggers restacking, any branch that is checked out in a different worktree is skipped. This means:

- Agent A's commit will NOT automatically rebase Agent B's branch
- Each agent must run `git-spice sr` in their own worktree to pick up upstream changes
- The stack may temporarily appear out of sync until all agents restack

### Branch Deletion

`git-spice branch delete` (and `git-spice repo sync`) will not delete branches that are checked out in other worktrees. Git prevents this at the filesystem level. To clean up:

1. Remove the worktree first: `git worktree remove ~/project.worktrees/part-1`
2. Then delete the branch: `git-spice bd feature-part-1`

Or let `git-spice repo sync` clean it up on the next run after the worktree is removed.
...
- `git-spice cc` / `git-spice ca` (both write to refs)
- `git-spice sr` (rebases may modify shared refs)
- `git-spice repo sync` (fetches and updates refs)
...
- **`git-spice stack submit` (`git-spice ss`)**: Pushes and creates/updates PRs for all branches in the stack. If two agents run this simultaneously, they may create duplicate PRs or encounter push conflicts.
- **`git-spice stack restack` (`git-spice sr`)**: If run from a worktree that is mid-stack, it will skip branches in other worktrees and may leave the stack partially restacked.
...
### Prefer git-spice Commands Over git Commands

This is always true, but especially important in worktree setups:

- `git-spice cc` over `git commit` -- maintains stack consistency and triggers restacking
- `git-spice bc` over `git checkout -b` -- tracks the branch in the stack
- `git-spice bd` over `git branch -d` -- rebases upstack onto the next downstack branch

### Verify Stack State Before and After Operations

Always run `git-spice ls` (or `git-spice ll` for more detail) before starting work and after completing operations:
...
Only one agent should run `git-spice stack submit` (`git-spice ss`) at a time. Recommended patterns:

1. **Designated submitter**: One agent (or the orchestrator) is responsible for all `git-spice ss` calls
...
3. **Branch-level submission** (preferred for multi-agent setups): Each agent submits only their own branch with `git-spice branch submit` (`git-spice bs`) instead of submitting the whole stack. This avoids force-pushing branches owned by other agents and allows CI to start per-branch. See the "Coordinated Restack Workflow" section for the full pattern including restack coordination and conflict ownership.

### Handling Merge Conflicts During Restack

When Agent B runs `git-spice sr` and encounters a conflict caused by Agent A's changes:

1. Resolve the conflict in the affected files
2. Stage resolved files with `git add`
3. Run `git-spice rebase continue` (`git-spice rbc`)
4. Verify with `git-spice ls` that the stack looks correct

If the conflict is too complex, abort with `git-spice rebase abort` (`git-spice rba`) and coordinate with the other agent or the user.

## Coordinated Restack Workflow

When multiple agents have branches checked out in separate worktrees across a stack, restacking requires explicit coordination. The standard `git-spice stack restack` skips branches in other worktrees, so a dedicated restack coordinator must orchestrate the process.
...
2. Run `git-spice stack restack` — this restacks any branches NOT checked out in other worktrees
3. For each branch checked out in a worktree (bottom to top), cd into that worktree and run:
   ...
   git-spice branch restack  # restacks just this branch onto its updated parent
   ```
...
Do NOT batch all pushes with a single `git-spice stack submit` at the end. Instead, each branch should be pushed individually as soon as it's restacked:

```bash
cd /path/to/repo.worktrees/<branch-worktree>
git-spice branch restack   # restack this branch
git-spice branch submit    # push immediately
```
...
1. The restack coordinator runs `git-spice branch restack` in a worktree
...
3. The branch owner resolves the conflict:
   ...
   ```
4. The branch owner then pushes their branch: `git-spice branch submit`
...
After `main` is updated (e.g., via `git-spice repo sync`):
...
git-spice bc feature-part-1
git-spice bc feature-part-2
git-spice bc feature-part-3
...
cd ~/project.worktrees/part-1
# ... implement part 1 ...
git-spice cc -m "Implement part 1"
git-spice ls  # verify stack state

# === Agent B (in ~/project.worktrees/part-2/) ===
cd ~/project.worktrees/part-2
git-spice sr  # restack to pick up Agent A's changes
# ... implement part 2 ...
git-spice cc -m "Implement part 2"
git-spice ls  # verify stack state

# === Agent C (in ~/project.worktrees/part-3/) ===
cd ~/project.worktrees/part-3
git-spice sr  # restack to pick up changes from A and B
# ... implement part 3 ...
git-spice cc -m "Implement part 3"
git-spice ls  # verify stack state

# === Submit (each agent pushes their own branch) ===
# Agent A:
cd ~/project.worktrees/part-1
git-spice bs --fill  # submit just this branch
