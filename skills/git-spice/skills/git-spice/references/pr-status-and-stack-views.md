...
How to list, inspect, and combine PR information for git-spice stacks using `git-spice` and `gh` CLI tools.

> Sources:
>
> - [git-spice CLI Reference](https://abhinav.github.io/git-spice/cli/reference/) -- `git-spice log short`, `git-spice log long`
...
### git-spice ls --json Output Format
...
### git-spice ll (long view)

Same as `git-spice ls` but includes individual commits per branch. Supports `--json` and `--all` flags.
...
## Combining git-spice ls with gh pr view

To build a rich stack view, combine `git-spice ls --json` (for stack structure and PR numbers) with `gh pr view` (for review status, CI checks, and metadata).

### Strategy

1. Run `git-spice ls --json -a` to get all branches with their CR numbers and stack structure
...
git-spice ls --json -a > /tmp/gs-stack.json
...
git-spice ls --json | jq -r 'select(.change) | .change.id | ltrimstr("#")' | while read -r num; do
